...
--   skill_pipeline.als          — this file: 49 checks, 19 runs (step ordering, deps, staleness, reports, gate audit)
...
--   PatternsMd       — alloy-patterns.md (48 patterns reference)
...
--   ReconciliationRpt — persisted reconciliation report (system-models/reports/)
--   EnforcementRpt    — persisted enforcement audit report (system-models/reports/)
--
-- Steps (total order, 14 steps):
...
--   S10b EnforcementAudit — verify Aligned rules are enforced at gate; check audit chain
--   S10  Iterate       — fix model or text based on reconciliation, re-run
--
-- Assertions (61 total across all modules):
--   THIS FILE (49 checks — require step ordering / artifact dependencies):
...
--   A55 ReportsPersistedToDisk      — reconciliation and enforcement reports written to disk
--   A56 ReconcileReportProducedByReconcile — report artifact tracks its producer
--   A57 EnforcementReportProducedByAudit  — enforcement report tracks its producer
--   A58 EnforcementAuditBeforeIterate     — audit happens before iterate
--   A59 GateAuditRequiredForEnforced      — Enforced verdicts must have audit chain
--   A60 GateAuditChainComplete            — all links in audit chain must be present
--   A61 ReportsStaleAfterIterate          — reports must be regenerated after model edit
--
...
-- These are the durable file artifacts; Reconciliation/Interpretation are in-conversation.
...
one sig Aligned          extends ReconcileOutcome {}  -- model matches source; enforce at gate
...
one sig NoChange_Aligned         extends FixDirection {}  -- Aligned: model and source agree
...
fact DirectionMatchesOutcome {
  all d: Discrepancy |
    (d.outcome = Aligned implies d.direction = NoChange_Aligned)
    and (d.outcome = FixSource implies d.direction = ModelStays_SourceChanges)
...
sig Discrepancy {
  ...
  gateAudit:      lone GateAuditChain,      -- only set for Enforced verdicts (step 10b-A)
...
-- ================================================================
-- GATE AUDIT CHAIN (Step 10b-A)
--
-- For each Enforced rule, trace whether the gate has auditable inputs.
-- A gate without auditable inputs checks claims, not evidence.
--
-- Four links in the chain:
--   1. What does the gate check? (gate condition)
--   2. Where is that data recorded? (recording location)
--   3. Does the recording format include the field? (field exists)
--   4. Does the collection step say to record it? (recording instruction)
--
-- Three failure modes:
--   MissingField        — format has no field for what the gate checks
--   MissingInstruction  — field exists but collection step doesn't say to fill it
--   AmbiguousField      — field exists but semantics are unclear (auditor can't verify)
-- ================================================================

abstract sig GateAuditStatus {}
one sig AuditOK              extends GateAuditStatus {}  -- all four links present and clear
one sig AuditMissingField    extends GateAuditStatus {}  -- format lacks the field
one sig AuditMissingInstruction extends GateAuditStatus {} -- field exists, no recording step
one sig AuditAmbiguousField  extends GateAuditStatus {}  -- field exists but ambiguous semantics

sig GateAuditChain {
  status:               one GateAuditStatus,
  fieldExists:          one Bool,     -- recording format has the field
  recordingInstruction: one Bool,     -- upstream step says to record it
  unambiguous:          one Bool      -- field semantics are deterministic
}

-- Audit status must be consistent with chain links
fact GateAuditConsistency {
  all g: GateAuditChain |
    -- OK: all three links are present
    (g.status = AuditOK implies
      (g.fieldExists = Yes and g.recordingInstruction = Yes and g.unambiguous = Yes))
    -- MissingField: format doesn't have the field
    and (g.status = AuditMissingField implies g.fieldExists = No)
    -- MissingInstruction: field exists but not told to fill it
    and (g.status = AuditMissingInstruction implies
      (g.fieldExists = Yes and g.recordingInstruction = No))
    -- AmbiguousField: field exists, instruction exists, but semantics unclear
    and (g.status = AuditAmbiguousField implies
      (g.fieldExists = Yes and g.recordingInstruction = Yes and g.unambiguous = No))
}

-- Gate audit only applies to Enforced discrepancies
fact GateAuditOnlyForEnforced {
  all d: Discrepancy |
    (d.enforcement = Enforced implies some d.gateAudit)
    and (d.enforcement != Enforced implies no d.gateAudit)
}

-- Every GateAuditChain belongs to exactly one Discrepancy
fact GateAuditOwned {
  all g: GateAuditChain | one d: Discrepancy | d.gateAudit = g
}
...
fact AlignedNeedsEnforcement {
  EnforcementConfig.sourceNature = NaturalLanguage implies
    all d: Discrepancy | d.outcome = Aligned implies some d.enforcement
}

-- Non-aligned discrepancies don't get enforcement (they're being fixed/resolved, not audited)
fact NonAlignedNoEnforcement {
  all d: Discrepancy | d.outcome != Aligned implies no d.enforcement
}
...
fact PatternConsultation {
  ...
  S10b_EnforcementAudit.readsPatterns = No  -- checks gate language, not patterns
...
--   (d) Reports stale after iterate → must re-reconcile + re-audit
--
-- Vector (d): after iterate edits UserModel, the reconciliation and
-- enforcement reports describe the OLD model's alignment. They must
-- be regenerated against the NEW model after re-running.
--
...
fact Versions {
  ...
  -- Reports: v1 from Reconcile/EnforcementAudit (based on UserModel v1)
  some v: ArtifactVersion | v.artifact = ReconciliationRpt and v.writtenBy = S9b_Reconcile and v.version = 1
  some v: ArtifactVersion | v.artifact = EnforcementRpt and v.writtenBy = S10b_EnforcementAudit and v.version = 1
...
fact FreshnessGuards {
  ...
  -- Reports must be regenerated after iterate changes UserModel
  -- ReconciliationRpt is stale when UserModel has been edited
  some fc: FreshnessCheck |
    fc.step = S9b_Reconcile and fc.compares = ReconciliationRpt and fc.against = UserModel

  -- EnforcementRpt is stale when UserModel has been edited
  some fc: FreshnessCheck |
    fc.step = S10b_EnforcementAudit and fc.compares = EnforcementRpt and fc.against = UserModel
}
...
-- A55: Reports are persisted to disk (both artifacts exist and are produced)
assert ReportsPersistedToDisk {
  some ReconciliationRpt
  and some EnforcementRpt
  and ReconciliationRpt.mutable = Yes   -- updated on re-run
  and EnforcementRpt.mutable = Yes
}

-- A56: Reconciliation report is produced by the reconciliation step
assert ReconcileReportProducedByReconcile {
  ReconciliationRpt.producedBy = S9b_Reconcile
  and lt[S9_Interpret, S9b_Reconcile]  -- interpretation before reconciliation
}

-- A57: Enforcement report is produced by the enforcement audit step
assert EnforcementReportProducedByAudit {
  EnforcementRpt.producedBy = S10b_EnforcementAudit
  and lt[S9b_Reconcile, S10b_EnforcementAudit]
}

-- A58: Enforcement audit must happen before iterate
assert EnforcementAuditBeforeIterate {
  lt[S10b_EnforcementAudit, S10_Iterate]
  and EnforcementRpt in S10_Iterate.requires
}

-- A59: Every Enforced verdict has a gate audit chain
assert GateAuditRequiredForEnforced {
  all d: Discrepancy | d.enforcement = Enforced implies some d.gateAudit
}

-- A60: A passing gate audit means all chain links are present
assert GateAuditChainComplete {
  all d: Discrepancy | d.enforcement = Enforced and some d.gateAudit implies
    (d.gateAudit.status = AuditOK implies
      (d.gateAudit.fieldExists = Yes
        and d.gateAudit.recordingInstruction = Yes
        and d.gateAudit.unambiguous = Yes))
}

-- A61: Reports are stale after iterate (model changed → reports describe old model)
assert ReportsStaleAfterIterate {
  ReconciliationRpt.mutable = Yes
  and EnforcementRpt.mutable = Yes
  -- Freshness guards exist: reconcile/audit must compare reports against current UserModel
  and some fc: FreshnessCheck | fc.step = S9b_Reconcile and fc.compares = ReconciliationRpt and fc.against = UserModel
  and some fc: FreshnessCheck | fc.step = S10b_EnforcementAudit and fc.compares = EnforcementRpt and fc.against = UserModel
}
...
check DependenciesSatisfied      for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check SingleProducer             for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ModelRequiresPatterns      for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check RunRequiresModel           for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check RunRequiresTooling         for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check InterpretRequiresOutput    for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ExamplesNotRequiredForRun  for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ToolingIndependentOfSkill  for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check PatternsSyncedWithExamples for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check SkillMdCoversAllCategories for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check OutputIncludesChecks       for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check IterateRequiresInterpret   for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check InterpretExplainsCounterexamples for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReviewBeforeRun            for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReviewRequiresModel        for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check RunOutputStaleAfterIterate for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ModelRequiresClarifiedInput for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ToolingUsesExactlyOneRuntime for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check RuntimeChoiceResolved      for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check BoundaryReviewHasProofOfWork for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReconcileRequiresInterpretAndSpec for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReconcileBeforeIterate     for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check FixModelImpliesRerun       for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReconciliationNonTrivial   for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReportOnlyIsValid          for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ModelRequiresSourceSystem  for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReconcileRequiresIncludedSources for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check BoundaryReviewRequiresSource for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReconcileChecksIncludedSources for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check SkippedSourcesNotChecked   for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ModelRequiresCode          for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReportHasBidirectionalRefs for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReportIsActionable         for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check QualityGateAtBoundaryReview for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check VaguePromptRequiresClarification for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ExampleSelectionRespectsBudget for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ExampleSelectionModeled    for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check IterationNotALoop          for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check EnforcementAuditRequired   for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check CodeSkipsEnforcementAudit  for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ConflictRequiresUserDecision for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ConflictDoesNotImplyRerun for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReportsPersistedToDisk    for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check ReconcileReportProducedByReconcile for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check EnforcementReportProducedByAudit for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check EnforcementAuditBeforeIterate for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
check GateAuditRequiredForEnforced for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
...
} for 3 but 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
...
-- S8: All six reconciliation outcomes co-exist
run AllReconcileOutcomes {
  some disj d1, d2, d3, d4, d5, d6: Discrepancy |
    d1.outcome = Aligned             -- model matches source → enforce
    and d2.outcome = FixSource       -- model correct → fix source
    and d3.outcome = FixModelToCode  -- code changed → model tracks as-is
    and d4.outcome = FixModelToIntent -- spec updated → model tracks intent
    and d5.outcome = Conflict        -- sources contradict → user decides
    and d6.outcome = Exclusion       -- conscious exclusion
  and EnforcementConfig.sourceNature = NaturalLanguage
} for 3 but exactly 6 Discrepancy, 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
...
} for 3 but exactly 2 Discrepancy, 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
...
} for 5 but exactly 1 Discrepancy, 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 7 ArtifactVersion, 6 Int
...
-- ================================================================
-- REPORT & GATE AUDIT SCENARIOS
-- ================================================================

-- S15: Gate audit passes — all links in chain present
run GateAuditAllOK {
  some d: Discrepancy |
    d.enforcement = Enforced
    and d.gateAudit.status = AuditOK
    and d.gateAudit.fieldExists = Yes
    and d.gateAudit.recordingInstruction = Yes
    and d.gateAudit.unambiguous = Yes
  and EnforcementConfig.sourceNature = NaturalLanguage
} for 3 but exactly 1 Discrepancy, 1 GateAuditChain, 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion

-- S16: Gate audit fails — missing field
run GateAuditMissingField {
  some d: Discrepancy |
    d.enforcement = Enforced
    and d.gateAudit.status = AuditMissingField
    and d.gateAudit.fieldExists = No
  and EnforcementConfig.sourceNature = NaturalLanguage
} for 3 but exactly 1 Discrepancy, 1 GateAuditChain, 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion

-- S17: Gate audit fails — field exists but no recording instruction
run GateAuditMissingInstruction {
  some d: Discrepancy |
    d.enforcement = Enforced
    and d.gateAudit.status = AuditMissingInstruction
    and d.gateAudit.fieldExists = Yes
    and d.gateAudit.recordingInstruction = No
  and EnforcementConfig.sourceNature = NaturalLanguage
} for 3 but exactly 1 Discrepancy, 1 GateAuditChain, 5 PatternCoverage, 5 ExampleRelevance, 4 ProofOfWork, 6 ArtifactVersion
